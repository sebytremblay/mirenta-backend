...
- `livekit_agent/` (LiveKit Cloud Agents worker), for **live inbound calls** — native LiveKit `AgentSession` (Deepgram STT/TTS + OpenAI LLM), dispatched when FastAPI prepares a room; not a Temporal activity.
...
1. Creates a LiveKit room with Mirenta correlation metadata (`org_id`, `contact_id`, `signal_id`, `call_sid`) and explicitly dispatches the named agent (`settings.LIVEKIT_AGENT_NAME`, default `mirenta-voice`).
2. Returns TwiML `<Dial answerOnBridge="true"><Sip>sip:{room}@{LIVEKIT_SIP_HOST};transport=tcp</Sip></Dial>` so Twilio bridges the PSTN leg into that room.
3. The Cloud-hosted worker (`livekit_agent/src/agent.py`) bootstraps instructions + org knowledge via `POST /api/v1/internal/voice/bootstrap`, runs Deepgram STT → OpenAI LLM → Deepgram TTS with LiveKit barge-in/VAD, then on shutdown calls `POST /api/v1/internal/voice/finalize`, which invokes `log_interaction` / `emit_interaction_result_signal` and re-enters `ContactLoopWorkflow` — the same close-the-loop step SMS uses.
...
| LiveKit client    | `app/services/clients/livekit_client.py`    | Create room + agent dispatch; SIP URI for Twilio Dial                                                   |
