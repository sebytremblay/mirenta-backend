...
Mirenta runs on the **Takeoff Runtime** — an event-driven, durable-workflow architecture for agent work that spans days to weeks across voice and SMS. LLMs touch only the conversation surface; everything that decides _when, whether, and on what channel_ to act is deterministic code. That split is what makes long-running, multi-channel outreach predictable and auditable.

> **Implementation status.** The full Temporal-driven loop — signal ingestion, the event bus (Temporal signal-with-start), the deterministic decision engine, durable task scheduling, and the interaction layer — runs end-to-end for **SMS**. **Inbound voice calls** are also live, but via a parallel path: a live phone call is a long-lived duplex audio session that doesn't fit inside a bounded Temporal activity, so the call itself is answered and conversed over outside Temporal (`app/services/voice_runtime.py`, bridging Twilio Media Streams + Deepgram STT/TTS to a voice LangGraph subagent); only the terminal "log the interaction, re-enter the loop" step rejoins the Temporal-driven `ContactLoopWorkflow`, exactly like SMS's own closing step. Outbound calling (the `call` task type) is not implemented — `TaskExecutionWorkflow` still rejects any non-`sms` task. Proactive/first-touch outreach and auto-follow-up rules also don't exist yet. See [Component status](#component-status) for the precise breakdown.
...
- `app/services/voice_runtime.py`'s `VoiceCallSession`, for **live inbound calls** — invoked once per conversational turn, directly from the WebSocket handler, not from a Temporal activity (see below for why).
...
**Why a live call can't be a Temporal activity the way an SMS task is:** `BaseChannelAgent`'s whole invocation model — and Temporal's activity model generally — is "one bounded call in, one result out." A phone call is a single long-lived duplex audio session (Twilio Media Streams pushes continuous 8kHz mulaw audio frames over a WebSocket for the call's entire duration), which doesn't fit inside a bounded activity, and Temporal workflow code can't do raw I/O at all. So `app/services/voice_runtime.py`'s `VoiceCallSession` owns the entire call outside Temporal:

1. `POST /webhooks/twilio/voice` answers the call with TwiML pointing at `WS /api/v1/ws/twilio/voice/{call_sid}`, carrying org/contact/signal correlation as `<Stream>` `<Parameter>`s (echoed back in Twilio's `start` event, since the WS handshake itself carries no query context).
2. `VoiceCallSession` bridges Twilio's Media Stream audio ↔ Deepgram streaming STT (`app/services/deepgram_client.py::DeepgramSTTSession`) ↔ one turn of `voice_agent.get_response(...)` per finalized utterance ↔ Deepgram streaming TTS (`DeepgramTTSSession`) ↔ audio back to Twilio, with barge-in (Deepgram's `SpeechStarted` VAD event cancels in-flight TTS playback and sends Twilio a `clear` message).
...
| Subagents                  | LangGraph graphs per channel                                                   | SMS done (`sms_graph.py`, invoked from a Temporal activity); voice done (`voice_graph.py`, invoked live from `voice_runtime.py`), inbound only |
...
| Twilio voice webhook + Media Stream bridge | `app/api/routers/voice.py`, `app/services/voice_runtime.py`               | Implemented, inbound only — resolves DNC/consent synchronously, doesn't route through the decision engine |
| Deepgram STT/TTS client                    | `app/services/deepgram_client.py`                                         | Implemented — streaming STT + Aura streaming TTS, 8kHz mulaw           |
...
| `ContactLoopWorkflow` / `TaskExecutionWorkflow` | `workflows/contact_loop.py`, `workflows/task_execution.py`            | Implemented for the `sms` task type; voice calls don't use `TaskExecutionWorkflow` at all (see §6) |
...
| Org phone auto-provisioning                | `app/services/twilio_client.py::provision_phone_number`                   | Implemented — wires both `sms_url` and `voice_url` on the purchased number |
...
| Voice runtime      | `app/services/voice_runtime.py`             | `VoiceCallSession` — bridges one live call's audio to Deepgram + the voice subagent, outside Temporal        |
| Deepgram client    | `app/services/deepgram_client.py`           | Streaming STT session + per-turn streaming TTS, 8kHz mulaw for Twilio Media Streams                          |
...
| Twilio client      | `app/services/twilio_client.py`            | Outbound SMS send, TwiML generation for voice answer/reject, available-number search, number purchase       |
