...
New organizations get a Twilio number provisioned automatically at creation (`provision_phone_number` in `app/services/clients/twilio_client.py`, wired into `POST /api/v1/organizations`) so `organizations.phone` — the field both SMS and voice route against — is populated without a manual step, with both the SMS and voice webhook URLs wired on the purchased number. This is best-effort: a Twilio failure doesn't fail org creation, and the number can be set manually afterward via `PATCH /api/v1/organizations/{id}`.
...
| Org phone auto-provisioning                | `app/services/clients/twilio_client.py::provision_phone_number`                   | Implemented — wires both `sms_url` and `voice_url` on the purchased number |
...
| Twilio client      | `app/services/clients/twilio_client.py`            | Outbound SMS send, TwiML generation for voice answer/reject, available-number search, number purchase       |
