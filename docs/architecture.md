...
- `livekit_agent/` (LiveKit Cloud Agents worker), for **live inbound calls** — native LiveKit `AgentSession` (Deepgram STT/TTS + OpenAI LLM), dispatched by the LiveKit inbound trunk + individual dispatch rule when Twilio SIP-dials in; not a Temporal activity.
...
**Voice (LiveKit Cloud agent)** — FastAPI keeps the Twilio numbers and the DNC/consent gate. On an allowed call it:

1. Builds a SIP dial URI for the org's E.164 number on `LIVEKIT_SIP_HOST`, carrying Mirenta correlation as Twilio custom SIP headers (`X-Mirenta-Org-Id`, `X-Mirenta-Contact-Id`, `X-Mirenta-Signal-Id`, `X-Mirenta-Call-Sid`).
2. Returns TwiML `<Dial answerOnBridge="true"><Sip>sip:+{org_phone}@{LIVEKIT_SIP_HOST};transport=tcp?x-mirenta-…</Sip></Dial>` so Twilio bridges the PSTN leg into LiveKit.
3. LiveKit's **inbound trunk** (matched on the dialed E.164) maps those headers to participant attributes via `headers_to_attributes`; an **individual dispatch rule** creates a `call-*` room and dispatches the named agent (`settings.LIVEKIT_AGENT_NAME`, default `mirenta-voice`).
4. The Cloud-hosted worker (`livekit_agent/src/agent.py`) bootstraps instructions + org knowledge via `POST /api/v1/internal/voice/bootstrap`, runs Deepgram STT → OpenAI LLM → Deepgram TTS with LiveKit barge-in/VAD, then on shutdown calls `POST /api/v1/internal/voice/finalize`, which invokes `log_interaction` / `emit_interaction_result_signal` and re-enters `ContactLoopWorkflow` — the same close-the-loop step SMS uses.

**Why a live call can't be a Temporal activity the way an SMS task is:** Temporal's activity model is "one bounded call in, one result out." A phone call is a long-lived duplex audio session. LiveKit Agents (hosted on LiveKit Cloud) own that session; Temporal only sees the terminal interaction result. FastAPI stays out of the media path so API/webhook capacity is independent of concurrent call count.

A crash between hangup and finalize completing can lose that one interaction's logging/loop re-entry — an accepted gap for this pass, since the live call itself is already lost on crash regardless.
...
| Subagents                  | LangGraph (SMS) + LiveKit Agents (voice)                                   | SMS done (`sms_graph.py`, knowledge-grounded, Temporal activity); voice done (`livekit_agent/`, Deepgram+OpenAI on LiveKit Cloud), inbound only |
...
| Twilio voice webhook + LiveKit SIP dial    | `app/api/routers/voice.py`, `app/services/clients/livekit_client.py`     | Implemented, inbound only — DNC/consent in webhook, then Dial Sip into LiveKit |
| LiveKit Cloud voice agent                  | `livekit_agent/`                                                          | Implemented — Deepgram STT/TTS + OpenAI LLM; bootstrap/finalize via FastAPI |
...
| LiveKit voice agent | `livekit_agent/`                          | Cloud-hosted Deepgram+OpenAI voice pipeline; Mirenta bootstrap/finalize HTTP bridge                     |
| LiveKit client    | `app/services/clients/livekit_client.py`    | SIP URI builder for Twilio Dial (room + agent dispatch owned by LiveKit trunk/rule)                       |
...
| Twilio client      | `app/services/clients/twilio_client.py`            | Per-org subaccount/Messaging Service provisioning, outbound SMS (MS SID preferred), TwiML for voice Dial Sip / reject |
