...
**`task.type == "sms"` and `task.type == "email"` execute.** An `sms` task runs the LangGraph SMS subagent; an `email` task runs the deterministic post-meeting follow-up (composed by `app.services.email.build_post_meeting_email`, sent via the org's connected Google account — no LLM). The one email task in use today is the post-meeting thank-you, emitted by `decide_on_meeting_scheduled` and scheduled for the meeting's end time. Any other task type (`call`, `webhook`, `api_call`) hits `TaskExecutionWorkflow`'s type check and is marked `status="failed"` immediately — there's no send-side implementation for those channels yet (see §6).
