...
- ``schedule_meeting`` — the single booking action; writes the calendar event
  and texts the caller a confirmation in one call.
"""
...
def build_scheduling_tools(
    *,
    ...
    async def schedule_meeting(
        ...
        """Book a meeting the caller chose and text them a confirmation.
        ...
        example a listing address) when you know it.
        ...
        if result.get("sms_sent", False):
            return f"You are booked for {label}. I just texted you a confirmation with the details."
        return f"You are booked for {label}. I could not send a text confirmation, but the meeting is on the calendar."
